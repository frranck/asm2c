...
R(MOV(32,READDD(eax),32,(dd)(1024*1024)));
...
R(MOV(8,*((db *) realAddress(0, ds)),8,(db)55));
R(CMP(8,*((db *) realAddress(0, es)),8,(db)55));
R(JE(failure));
R(MOV(8,*((db *) realAddress(0, es)),8,(db)56));
R(INC(8,*((db *) realAddress(0, es))));
R(CMP(8,*((db *) realAddress(0, es)),8,(db)57));
R(JNE(failure));
R(LEA(32,m.esi.dd.val,32,(((dd)offsetof(struct Mem,f)))));
R(PUSH(16,(READDW(es))));
R(POP(16,(READDW(ds))));
R(LEA(32,m.edi.dd.val,32,(((dd)offsetof(struct Mem,f)))));
...
fprintf(file, "xox %x (from beg RW) %x:pas_de_mem\n",(unsigned int) offsetof(struct Mem,pas_de_mem)-offset,(unsigned int) offsetof(struct Mem,pas_de_mem));
fprintf(file, "xox %x (from beg RW) %x:pbs1\n",(unsigned int) offsetof(struct Mem,pbs1)-offset,(unsigned int) offsetof(struct Mem,pbs1));
fprintf(file, "xox %x (from beg RW) %x:pbs2\n",(unsigned int) offsetof(struct Mem,pbs2)-offset,(unsigned int) offsetof(struct Mem,pbs2));
